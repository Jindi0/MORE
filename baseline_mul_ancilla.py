...
from model import build_qnn_baseline_ancilla
...
def args_parser():
    ...
    parser.add_argument('--task', type=str, default='ancilla_012_8q', help='task name:[modelType]_[classes]_[modelSize]q')
    parser.add_argument('--ancilla', type=int, default=3, help='the number of ancilla qubits')
    
    parser.add_argument('--samples', type=int, default=2000, help='the number of training samples')
    parser.add_argument('--COBYLAiter', type=int, default=200,help="the number of epochs")
...
savepath = './MORE/save_baseline_8q_mul_ancilla/'
if not os.path.exists(savepath):
    os.mkdir(savepath)
task_path = './MORE/save_baseline_8q_mul_ancilla/{}/'.format(args.task)
...
def onehot_label(ys, classes):  
    ...
    new_ys = np.zeros((len(ys), args.ancilla))
...
if __name__ == "__main__":
    ...
    qnn = build_qnn_baseline_ancilla(args.ancilla)
